...
public class TKOLift // implements Runnable is important

// to make this class support the
// Thread (run method)
{
	private static Thread liftThreadAuto;
	 ...
	 */
	private TKOThread liftThread = null;
	private double p = SmartDashboard.getNumber("Lift P: ", Definitions.LIFT_P);
	private double i = SmartDashboard.getNumber("Lift I: ", Definitions.LIFT_I);
	private double d = SmartDashboard.getNumber("Lift D: ", Definitions.LIFT_D);
	private double count = 0;
	private double total = 0;
	private volatile double elon = 0;
	...
	public void tele() {
		Thread liftThreadTele = new Thread(this::bill);
	...
	@Deprecated
	public void pidLiftDeprecated(int setpoint, int threshold) {
		try {
			Timer t = new Timer();
			t.reset();
			t.start();
